...
class OllamaService:
    def __init__(self):
        ...
        self.backoff = settings.retry_backoff
    ...
    async def _request_with_retry(self, method: str, path: str, **kwargs) -> httpx.Response:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.warning(
                    "Ollama %s %s attempt %d/%d failed: %s — retrying in %.1fs",
                    method.upper(), path, attempt + 1, self.max_retries, e, wait,
                )
    ...
    async def chat_stream(
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                wait = self.backoff * (2 ** attempt)
                logger.warning(
                    "Chat stream attempt %d/%d failed: %s — retrying in %.1fs",
        ...
        raise RuntimeError(
            f"Chat stream failed after {self.max_retries} attempts: {last_exc}"
        )
